            ...
            <div class="ticket-card rounded-3xl p-6 lg:p-8">
                ...
                <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 border-b border-slate-200 pb-5">
                    <div>
                        ...
                        <h2 class="mt-2 text-2xl lg:text-3xl font-black text-slate-900">{{ $pedido['codigo'] }}</h2>
                        <p class="mt-2 text-sm text-slate-500">Fecha: {{ $pedido['fecha'] }}</p>
                    </div>
                    <div class="rounded-2xl bg-emerald-600 px-4 py-3 text-white lg:text-right">
                        ...
                        <p class="text-xl font-black">{{ $pedido['estado'] }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-6">
                    <section class="lg:col-span-8">
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                            <div class="flex items-center justify-between gap-3 mb-4">
                                ...
                                <span class="rounded-full bg-slate-900 px-3 py-1 text-xs font-bold text-white">{{ count($pedido['detalles']) }} artículos</span>
                            </div>
                                ...
                                @foreach($pedido['detalles'] as $linea)
                                    <article class="rounded-2xl bg-white p-4 border border-slate-200 shadow-sm">
                                        <div class="flex items-start justify-between gap-4">
                                            <div>
                                                <h4 class="font-bold text-slate-900">{{ $linea['nombre'] }}</h4>
                                                <p class="text-sm text-slate-500 mt-1">{{ $linea['cantidad'] }} x {{ number_format($linea['precio_unitario'], 2, ',', '.') }} €</p>
                                            </div>
                                            <div class="text-right">
                                                <p class="text-[10px] uppercase tracking-[0.24em] text-slate-400 font-bold">Subtotal</p>
                                                <p class="text-lg font-black text-slate-900">{{ number_format($linea['subtotal'], 2, ',', '.') }} €</p>
                                            </div>
                                        </div>
                                    </article>
                                @endforeach
                    ...
                    <aside class="space-y-4 lg:col-span-4">
                        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            ...
                            <h3 class="mt-2 text-xl font-black text-slate-900">{{ strtoupper($pedido['metodo_pago']) }}</h3>
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            ...
                            <div class="mt-4 space-y-3">
                                <a href="{{ route('pedido.pdf', ['id' => $pedido['codigo']]) }}" class="inline-flex w-full items-center justify-center rounded-2xl bg-slate-900 px-4 py-3 text-sm font-bold text-white transition hover:bg-slate-800">
                                    Descargar PDF
                                </a>
                        ...
                        <form method="POST" action="{{ route('pedido.enviar', ['id' => $pedido['codigo']]) }}" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            @csrf
                            ...
                            <input id="correo" name="correo" type="email" value="{{ $pedido['correo_ticket'] ?? '' }}" class="mt-2 w-full rounded-xl border-slate-300 focus:border-emerald-500 focus:ring-emerald-500" required>
                            @error('correo')
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-slate-900 p-5 text-white shadow-sm">
                            ...
                            <p class="mt-2 text-4xl font-black">{{ number_format($pedido['total'], 2, ',', '.') }} €</p>
                        </div>
                        ...
                        <a href="{{ route('index') }}" class="inline-flex w-full items-center justify-center rounded-2xl bg-emerald-600 px-5 py-4 text-white font-black text-lg transition hover:bg-emerald-700">
                            Volver a la tienda
                        </a>
                    </aside>
                </div>
