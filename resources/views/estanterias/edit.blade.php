<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            🏗️ Editando Ubicación: {{ $estanteria->nombre }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-8 shadow-2xl rounded-3xl border border-gray-100">
                <form action="{{ route('estanterias.update', $estanteria->id) }}" method="POST">
                    @csrf 
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="nombre" value="{{ old('nombre', $estanteria->nombre) }}" 
                        ...
                        <div class="bg-indigo-50 p-4 rounded-2xl">
                            ...
                            <input type="number" name="x" value="{{ old('x', $estanteria->x) }}" 
                        ...
                        <div class="bg-indigo-50 p-4 rounded-2xl">
                            ...
                            <input type="number" name="y" value="{{ old('y', $estanteria->y) }}" 
                                   class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm" required>
                        ...
                        <div>
                            ...
                            <input type="number" name="ancho" value="{{ old('ancho', $estanteria->ancho) }}" 
                        ...
                        <div>
                            ...
                            <input type="number" name="alto" value="{{ old('alto', $estanteria->alto) }}" 
                                   class="mt-1 block w-full border-gray-300 rounded-xl shadow-sm" required>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 flex justify-between items-center border-t pt-6">
                        ...
                        <a href="{{ route('estanterias.index') }}" class="text-gray-400 hover:text-gray-600 font-medium text-sm">
                            ⬅ Cancelar y volver
                        </a>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6 p-6">
                <div class="rounded-2xl bg-slate-950 p-4 overflow-auto">
                    <svg id="preview-svg-edit" viewBox="0 0 800 600" class="h-[520px] w-full rounded-xl bg-slate-900 border border-slate-700">
                        ...
                        <g id="preview-group-edit" transform="translate({{ old('x', $estanteria->x) }} {{ old('y', $estanteria->y) }})">
                            <rect id="preview-new-edit" x="0" y="0" width="{{ old('ancho', $estanteria->ancho) }}" height="{{ old('alto', $estanteria->alto) }}" rx="4" fill="rgba(59, 130, 246, 0.65)" stroke="#3b82f6" stroke-width="4" />
                            <text id="preview-name-edit" x="6" y="16" fill="#ffffff" font-size="11" font-weight="700">{{ old('nombre', $estanteria->nombre) }}</text>
                            <text id="preview-xy-edit" x="6" y="31" fill="#ffffff" font-size="10">X:{{ old('x', $estanteria->x) }} Y:{{ old('y', $estanteria->y) }}</text>
                            <text id="preview-size-edit" x="6" y="45" fill="#ffffff" font-size="10">{{ old('ancho', $estanteria->ancho) }} x {{ old('alto', $estanteria->alto) }}</text>
                        </g>
    ...
    <script>
        ...
        const CURRENT_ESTANTERIA_ID = {{ $estanteria->id }};
