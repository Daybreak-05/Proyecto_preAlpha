<x-guest-layout width="7xl">
    ...
    <div class="min-h-screen checkout-bg text-slate-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
            <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-8">
                ...
                <a href="{{ route('index') }}" class="inline-flex items-center justify-center rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-sm font-bold text-white transition hover:bg-white/15">
                    Volver a la tienda
                </a>
            </div>
    ...
    <script>
        ...
        btnPagar.addEventListener('click', async function () {
            ...
            try {
                const response = await fetch('{{ route("carrito.procesar") }}', {
                    method: 'POST',
                    headers: {
