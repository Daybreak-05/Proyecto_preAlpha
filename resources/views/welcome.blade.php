...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'UBIKA') }} - Tienda</title>
    ...
    <div class="min-h-screen fondo-almacen relative">
        ...
        <div class="fixed top-4 right-4 z-0 pointer-events-none opacity-10">
            <img src="{{ asset('img/logo.png') }}" alt="" class="h-24 w-24">
        </div>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-white transition hover:bg-white/10 focus:outline-none">
                            <span class="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-400/15 text-emerald-300 font-bold">{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}</span>
       	...
       	<main class="w-full px-3 sm:px-6 lg:px-12 py-4 sm:py-6 lg:py-8">
            <div class="flex flex-col lg:flex-row gap-6 xl:gap-8 items-start">
                ...
                <div class="w-full lg:flex-1 tarjeta-mapa p-2 sm:p-4 md:p-6 rounded-3xl border overflow-x-auto">
                    @php $esRoot = Auth::check() && Auth::user()->isAdmin(); @endphp
                        ...
                        @foreach($estanterias as $e)
                            <g class="estanteria-group cursor-pointer" data-id="{{ $e->id }}" data-nombre="{{ $e->nombre }}">
                                ...
                                <text x="{{ $e->x + $e->ancho/2 }}" y="{{ $e->y + $e->alto/2 }}" 
                                    ...
                                    class="estanteria-label uppercase pointer-events-none select-none">{{ $e->nombre }}</text>
                            </g>
                        @endforeach
                    </svg>
                </div>
    ...
    <script>
        const IS_ROOT = "{{ Auth::check() && Auth::user()->email === 'root@root.es' ? '1' : '0' }}" === "1";
        ...
        document.querySelectorAll('.estanteria-group').forEach(group => {
			...
			`;
				...
				card.innerHTML = `
				    ${p.imagen ? `
				        <img 
				            src="{{ url('public/storage') }}/${p.imagen}" 
    ...
    <div class="max-w-7xl mx-auto px-6 py-10">
        ...
        <div class="border-t border-slate-800 mt-8 pt-6 text-center text-sm">
            © {{ date('Y') }} UBIKA · Proyecto Final de Desarrollo Web
        </div>
